...
class RedisManager:
    ...
    def __init__(self, main):
        self.main = main
        # smartshield picks a redis port from the following range
        self.start_port = 32768
        self.end_port = 32850
        self.running_logfile = "running_smartshield_info.txt"
            ...
            with open(self.running_logfile, "a") as f:
                ...
                if f.tell() == 0:
                    f.write(
                        ...
                        " Output Zeek Dir, Logs Dir, smartshield PID, Is Daemon, "
                        "Save the DB\n"
                    )
    ...
    def load_redis_db(self, redis_port):
        # to be able to use running_smartshield_info later as a non-root user,
    ...
    def load_db(self):
        ...
        self.main.terminate_smartshield()
    ...
    def close_all_ports(self):
        """
        Closes all the redis ports in running_smartshield_info.txt and
         in smartshield supported range of ports
        """
        ...
        # closes all the ports in smartshield supported range of ports
        smartshield_supported_range = list(range(self.start_port, self.end_port + 1))
        smartshield_supported_range.append(6379)
        for port in smartshield_supported_range:
    ...
    def print_port_in_use(self, port: int):
        print(
            ...
            f"\nOr kill your open redis ports using: ./smartshield.py -k "
        )

    def close_smartshield_if_port_in_use(self, port: int):
    ...
    def get_open_redis_servers(self) -> Dict[int, dict]:
        ...
        with PIDs and ports of the redis servers started by smartshield
        read from running_smartshield.info.txt
        """
                ...
                for line in f.read().splitlines():
                    ...
                    try:
                        ...
                        self.open_servers_pids[pid] = {
                            ...
                            "smartshield_pid": smartshield_pid,
                            "is_daemon": is_daemon,
                            "save_the_db": save_the_db,
                        }
                    except ValueError:
                        # sometimes smartshield can't get the server pid and logs "False"
    ...
    def get_redis_port(self) -> int:
        ...
        if all ports are unavailable, this function terminates smartshield
        """
        if self.main.args.port:
            redis_port = int(self.main.args.port)
            # if the default port is already in use, smartshield should override it
            if redis_port != 6379:
                # close smartshield if port is in use
                self.close_smartshield_if_port_in_use(redis_port)
            ...
            # even if this port is in use, it will be overwritten by smartshield
            redis_port = 6379
        return redis_port
    ...
    def flush_redis_server(self, pid: int = None, port: int = None):
        ...
        try:
            ...
            # if the redis server opened by smartshield is closed manually by the
            # user, not by smartshield, smartshield won't be able to connect to it
    ...
    def remove_old_logline(self, redis_port):
        ...
        tmpfile = "tmp_running_smartshield_log.txt"
    ...
    def close_open_redis_servers(self):
        ...
        with contextlib.suppress(KeyboardInterrupt):
            ...
            # close all ports in running_smartshield_logs.txt and in our supported range
