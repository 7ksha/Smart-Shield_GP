...
from smartshield_files.common.abstracts.imodule import (
    IModule,
)

from smartshield_files.common.style import green
from smartshield_files.core.evidence_handler import EvidenceHandler
from smartshield_files.core.helpers.bloom_filters_manager import BFManager
from smartshield_files.core.input import Input
from smartshield_files.core.output import Output
from smartshield_files.core.profiler import Profiler
...
class ProcessManager:
    def __init__(self, main):
        ...
        # shutdown at the very end of all other smartshield modules.
        self.evidence_handler_termination_event: Event = Event()
        self.stopped_modules = []
        # used to stop smartshield when these 2 are done
        # since the semaphore count is zero, smartshield.py will wait until another
        # thread (input and profiler)
        # release the semaphore. Once having the semaphore, then smartshield.py can
        # terminate smartshield.
    ...
    def set_smartshield_processes(self, children: Dict[str, Process]):
        # this will be set by main.py if smartshield is not daemonized,
    ...
    def start_output_process(self, stderr, smartshield_logfile, stdout=""):
        output_process = Output(
            stdout=stdout,
            stderr=stderr,
            smartshield_logfile=smartshield_logfile,
        ...
        self.smartshield_logfile = output_process.smartshield_logfile
    ...
    def kill_all_children(self):
        ...
        for process in self.processes:
            ...
            if not module_name:
                ...
                # by smartshield.py, we don't have it stored in
    ...
    def _change_cyst_module_order(self, plugins):
        # when cyst starts first, as soon as smartshield connects to cyst,
        # cyst sends smartshield the flows,
    ...
    def init_bloom_filters_manager(self):
        """this instance is shared accross all smartshield IModule instances,
    ...
    def start_update_manager(self, local_files=False, ti_feeds=False):
        ...
        PS; this function is blocking, smartshield.py will not start the rest of the
        ...
        try:
            ...
            # only one instance of smartshield should be able to update ports
            # and orgs at a time
            # so this function will only be allowed to run from 1 smartshield
            # instance.
            with Lock(name="smartshield_ports_and_orgs"):
            ...
            # another instance of smartshield is updating ports and orgs
            return
    ...
    def get_hitlist_in_order(self) -> Tuple[List[Process], List[Process]]:
        """
        returns a list of PIDs that smartshield should terminate first,
        ...
        # smartshield won't reach this function unless they are done already.
    ...
    def get_analysis_time(self) -> Tuple[str, str]:
        """
        Returns how long smartshield took to analyze the given file
        returns analysis_time in minutes and smartshield end_time as a date
        """
        start_time = self.main.db.get_smartshield_start_time()
    ...
    def stop_smartshield(self) -> bool:
        """
        determines whether smartshield should stop
        based on the following:
        1. is smartshield still receiving new flows? (checks input.py and
        profiler.py)
        2. did smartshield the control channel recv the stop_smartshield
    ...
    def is_stop_msg_received(self) -> bool:
        ...
        'stop_smartshield' msg
        This control channel is used by CYST or the filemanager to tell
        smartshield that zeek terminated (useful when running smartshield with -g)
        """
        ...
        return (
            ...
            and message["data"] == "stop_smartshield"
        )
    ...
    def should_run_non_stop(self) -> bool:
        """
        determines if smartshield shouldn't terminate because by default,
        it terminates when there's no more incoming flows
        """
        # these are the cases where smartshield should be running non-stop
        # when smartshield is reading from a special module other than the input process
        # this module should handle the stopping of smartshield
        return (
    ...
    def is_done_receiving_new_flows(self) -> bool:
        """
        Determines if smartshield is still receiving new flows.
    ...
    def kill_daemon_children(self):
        ...
        # they are the children of the smartshield.py that ran using -D
    ...
    def get_print_function(self):
        """
        returns the print() function to use based on the curr smartshield mode
        because the daemon's print isn't the same as the normal smartshield' print()
        """
        ...
        try:
            ...
            if self.main.mode == "daemonized":
                ...
                try:
                    ...
                    # or smartshield was stuck looping for too long that the OS
                    # sent an automatic sigint to kill smartshield
                    # pass to kill the remaining modules
                    reason = "User pressed ctr+c or smartshield was killed by the OS"
            ...
            # if store_a_copy_of_zeek_files is set to yes in smartshield.yaml
            # copy the whole zeek_files dir to the output dir
            self.main.store_zeek_dir_copy()

            # if delete_zeek_files is set to yes in smartshield.yaml,
            ...
            if graceful_shutdown:
                print(
                    "[Process Manager] smartshield shutdown gracefully\n",
                    log_to_logfiles_only=True,
                )
            else:
                print(
                    f"[Process Manager] smartshield didn't "
