...
class MetadataManager:
    ...
    def _add_metadata(self):
        ...
        smartshield.yaml, whitelist.conf, current commit and date
        """
        ...
        config_file = self.main.args.config or "config/smartshield.yaml"
    ...
    def set_analysis_end_date(self, end_date):
        ...
        # add smartshield end date in the metadata dir
        try:
            with open(self.info_path, "a") as f:
                f.write(f"smartshield end date: {end_date}\n")
    ...
    def set_input_metadata(self):
        ...
        info = {
            "smartshield_version": self.main.version,
    ...
    def update_smartshield_stats_in_the_db(self) -> Tuple[int, Set[str]]:
        """
        updates the number of processed ips, smartshield internal time,
         and modified tws so far in the db
        """
        smartshield_internal_time = float(self.main.db.get_smartshield_internal_time()) + 1
        ...
        ) = self.main.db.get_modified_profiles_since(smartshield_internal_time)
        ...
        # last_modified_tw_time is 0 the moment we start smartshield
        # or if we don't have modified tw since the last smartshield_internal_time
        if last_modified_tw_time != 0:
            self.main.db.set_smartshield_internal_time(last_modified_tw_time)
