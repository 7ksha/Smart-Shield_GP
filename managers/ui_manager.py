...
class UIManager:
    ...
    def check_if_webinterface_started(self):
        ...
        self.main.print(
            f"smartshield {green('web interface')} running on "
            f"http://localhost:{self.web_interface_port}/ "
            f"[PID {green(self.web_interface_pid)}]\n"
            f"The port will stay open after smartshield is done with the "
    ...
    def start_webinterface(self):
        ...
        def detach_child():
            """
            Detach the web interface from the parent process group(smartshield.py),
        ...
        def run_webinterface():
            ...
            # in smartshield not being able to
