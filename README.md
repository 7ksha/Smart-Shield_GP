...
![Docker Image Size (tag)](https://img.shields.io/docker/image-size/stratosphereips/smartshield/latest?color=blue&label=docker%20image%20size)
![Docker Pulls](https://img.shields.io/docker/pulls/stratosphereips/smartshield)
...
[![License](https://img.shields.io/badge/Blog-Stratosphere-cyan)](https://www.stratosphereips.org/blog/tag/smartshield)
...
# Our contributions vs. upstream

SmartShield inherits the full SLIPS detection engine, profiling, evidence
pipeline, Redis/SQLite storage, web interface, and Kalipso TUI **from upstream
SLIPS unchanged in spirit**. The work that is original to this graduation
project is the **OT/ICS (industrial) protection layer** and the integration and
tuning needed to make SLIPS useful in a factory / Operational-Technology
environment.

**Added by us**

| Area | What we added | Where |
|------|---------------|-------|
| OT detection module | New module that flags OT-specific attacks (Modbus flooding / illegal function codes, S7Comm STOP, DNP3 unauthorized functions, PLC mode-switching, valve/motor cycling, unauthorized access to protected OT devices) | `modules/ot_detection/ot_detection.py` |
| OT Zeek scripts | Load OT protocol analyzers (Modbus, DNP3) and emit OT events/notices for the module to consume | `zeek-scripts/ot_protocols.zeek` (loaded from `zeek-scripts/__load__.zeek`) |
| OT protocol channels | Redis channels `new_modbus`, `new_s7comm`, `new_dnp3` registered and wired to the module | `smartshield_files/core/database/redis_db/database.py` |
| OT configuration | Protected-device inventory, authorized Modbus masters, and an `ot_detection` config block with per-protocol flood thresholds | `config/ot_protected_devices.conf`, `config/modbus_authorized_masters.conf`, `config/smartshield.yaml` |
| OT tuning | `analysis_direction: all` and a lowered evidence threshold so inbound ICS attacks alert earlier | `config/smartshield.yaml` |
| Rebrand | User-facing product renamed SLIPS → SmartShield while preserving GPL + attribution | repo-wide, `NOTICE` |

**From upstream SLIPS (not our work)**

Threat-Intelligence, PortScan, RNN C&C detection, Flow Alerts, ARP, HTTP
Analyzer, Leak Detector, Update Manager, IP Info, Blocking, the profiler /
evidence / alert pipeline, the web interface, and Kalipso.

> _Note for the team: expand each row above with the specifics you implemented
> in semester 2 (detection logic, thresholds, test cases, evaluation results).
> Keep this table honest — examiners value a precise "we extended X to do Y"
> far more than a vague claim of authorship over the whole system._

---


(`LICENSES/GPL-2.0-only.txt`). It is a derivative work of
[Stratosphere Linux IPS (SLIPS)](https://github.com/stratosphereips/StratosphereLinuxIPS),
© Stratosphere Laboratory, FEL, Czech Technical University in Prague, which is
distributed under the same license. Upstream copyright notices and the original
license are retained as required by the GPL; see the `NOTICE` file for details.
...
# Demos
The following videos contain demos of smartshield in action in various events:

- 2022 BlackHat Europe Arsenal, smartshield: A Machine-Learning Based, Free-Software, Network Intrusion Prevention System [[web](https://www.blackhat.com/eu-22/arsenal/schedule/index.html#smartshield-free-software-machine-learning-tool-for-network-intrusion-prevention-system-29599)]
- 2022 BlackHat USA Arsenal, smartshield: A Machine-Learning Based, Free-Software, Network Intrusion Prevention System [[web](https://www.blackhat.com/us-22/arsenal/schedule/index.html#smartshield-free-software-machine-learning-tool-for-network-intrusion-prevention-system-26687)]
- 2021 BlackHat Europe Arsenal, smartshield: A Machine-Learning Based, Free-Software, Network Intrusion Prevention System [[slides](https://mega.nz/file/EAIjWA5D#DoYhJknH1hpbqfS2ayVLwA7ewNT50jFQb7S3dVAKPko)] [[web](https://www.blackhat.com/eu-21/arsenal/schedule/#smartshield-a-machine-learning-based-free-software-network-intrusion-prevention-system-25116)]
- 2021 BlackHat USA Arsenal, smartshield: A Machine-Learning Based, Free-Software, Network Intrusion Prevention System [[web](https://www.blackhat.com/us-21/arsenal/schedule/#smartshield-a-machine-learning-based-free-software-network-intrusion-prevention-system-24105)]
- 2021 BlackHat Asia Arsenal, smartshield: A Machine-Learning Based, Free-Software, Network Intrusion Prevention System [[web](https://www.blackhat.com/asia-21/arsenal/schedule/#smartshield-a-machine-learning-based-free-software-network-intrusion-prevention-system-22576)]
...
the value smartshield brings to the community.
