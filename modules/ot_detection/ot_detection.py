...
class OTDetection(IModule):
    ...
    def _read_configuration(self):
        ...
        # Load whitelisted Modbus masters (IPs allowed to read PLCs)
            ...
            if alert_key not in self._alerted:
                ...
                self._set_evidence(
                    ...
                    description=(
                        ...
                        f"{srcip} to PLC {dstip}: function 0x{function:02X} — "
    ...
    def _set_evidence(
        ...
        evidence = Evidence(
            ...
            ) if timestamp else utils.convert_ts_format(
                str(time.time()), utils.alerts_format
            ),
            victim=victim,
            proto=proto or Proto.TCP,
            dst_port=port,
