...
class Timeline(IModule):
    ...
    def convert_timestamp_to_smartshield_format(self, timestamp: float) -> str:
    ...
    def is_inbound_traffic(self, flow) -> bool:
        ...
        if self.analysis_direction != "all":
            # smartshield only detects inbound traffic in the "all" direction
            return False
    ...
    def process_tcp_udp_flow(self, flow):
        ...
        if not flow.dport_name:
            flow.dport_name = "????"
            critical_warning_dport_name = (
                "Protocol not recognized by smartshield nor Zeek."
            )
    ...
    def process_flow(self, profileid, twid, flow):
        ...
        try:
            ...
            flow.timestamp_human = self.convert_timestamp_to_smartshield_format(
                flow.starttime
            )
            flow.dur = round(float(flow.dur), 3)
            # interpret the given flow and and create an activity line to
            # display in smartshield Web interface/Kalipso
