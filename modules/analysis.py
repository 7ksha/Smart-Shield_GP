...
def get_ip_info(ip):
    ...
    if ip_info := db.get_ip_info(ip):
        ...
        threatintel_info = [
            (threatintel.get("description", "-") + "," + threatintel.get("threat_level", "-") + " threat level")
            if threatintel else "-"
        ]
...
def set_profile_tws():
    ...
    data = [{"profile": profile_ip, "blocked": blocked_state} for profile_ip, blocked_state in profiles_dict.items()]
    return {"data": data}
...
def set_tws(ip):
    ...
    for tw in blocked_tws:
        tws[tw]["blocked"] = True
    data = [{"tw": tw_value["tw"], "name": tw_value["name"], "blocked": tw_value["blocked"]} for tw_key, tw_value in tws.items()]
    return {"data": data}
...
def set_timeline_flows(ip, timewindow):
    data = []
    profileid = f"profile_{ip}"
    if timeline_flows := db.get_all_flows_in_profileid_twid(profileid, timewindow):
        for key, value in timeline_flows.items():
            ...
            if "ts" in value:
                ...
                value["ts"] = "-"  # or any default placeholder

            # check if 'dur' exists
            if "dur" in value:
                try:
                    value["dur"] = "{:.5f}".format(float(value["dur"]))
    ...
    if timeline := db.get_profiled_tw_timeline(profileid, timewindow):
        for flow in timeline:
            ...
            if flow.get("dport_name") == "IGMP":
                fields = ["dns_resolution", "dport/proto", "state", "sent", "recv", "tot", "warning", "critical"]
                for field in fields:
                    flow[field] = "????"
            if flow.get("preposition") == "from":
                flow["daddr"] = flow["saddr"]
            data.append(flow)
    return {"data": data}
...
def set_alerts(ip, timewindow):
    ...
    if alerts := db.get_profileid_twid_alerts(profile, timewindow):
        ...
        for alert_id, evidence_id_list in alerts_tw.items():
            ...
            evidence_details: dict = evidence[alert_id]
            if isinstance(evidence_details, str):
                evidence_details = json.loads(evidence_details)
            timestamp: str = ts_to_date(evidence_details["timestamp"], seconds=True)
...
def set_evidence(ip, timewindow, alert_id: str):
    data = []
    profileid = f"profile_{ip}"
    evidence_ids: List[str] = db.get_evidence_causing_alert(profileid, timewindow, alert_id)
    if evidence_ids:
        for evidence_id in evidence_ids:
            evidence: Dict[str, str] = db.get_evidence_by_id(profileid, timewindow, evidence_id)
