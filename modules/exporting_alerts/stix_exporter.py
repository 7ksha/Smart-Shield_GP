...
from smartshield_files.common.abstracts.iexporter import IExporter
...
class StixExporter(IExporter):
    ...
    def _load_existing_bundle(self) -> None:
        ...
        without creating duplicates if smartshield was restarted.
        """
    ...
    def _extract_evidence_id(self, indicator: Indicator) -> Optional[str]:
        try:
            return indicator.get("x_smartshield_evidence_id")  # type: ignore[index]
    ...
    def create_collection(self):
        if not self.collection_name:
            self.print(
                "collection_name is missing in smartshield.yaml; cannot export STIX.",
                0,
                3,
            )
            return None
    ...
    def read_configuration(self) -> bool:
        ...
        # push_delay is only used when smartshield is running using -i
    ...
    def _build_custom_properties(
        ...
        custom_properties: Dict[str, object] = {
            "x_smartshield_evidence_id": evidence.get("id"),
            "x_smartshield_threat_level": evidence.get("threat_level"),
            "x_smartshield_profile_ip": profile.get("ip"),
            "x_smartshield_timewindow": timewindow.get("number"),
            "x_smartshield_attacker_direction": attacker.get("direction"),
            "x_smartshield_attacker_ti": attacker.get("TI"),
            "date_added": date_added,
        }

        victim_value = victim.get("value")
        if victim_value:
            custom_properties["x_smartshield_victim"] = victim_value

        uids = evidence.get("uid")
        if uids:
            custom_properties["x_smartshield_flow_uids"] = uids

        dst_port = evidence.get("dst_port")
        if dst_port:
            custom_properties["x_smartshield_dst_port"] = dst_port

        src_port = evidence.get("src_port")
        if src_port:
            custom_properties["x_smartshield_src_port"] = src_port
    ...
    def add_to_stix_file(self, evidence: dict) -> bool:
        ...
        indicator = Indicator(
            name=evidence.get("evidence_type", "smartshield Alert"),
    ...
    def schedule_sending_to_taxii_server(self):
        ...
        """
        while True:
            # on an interface, we use the push delay from smartshield.yaml
            # on files, we push once when smartshield is stopping
