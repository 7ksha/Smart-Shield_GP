...
class SlackExporter(IExporter):
    ...
    def send_init_msg(self):
        self.export(
            ...
            f"smartshield started on sensor: {self.sensor_name}."
        )

    def send_stop_msg(self):
        self.export(
            f"{utils.get_human_readable_datetime()}: "
            f"smartshield stopped on sensor: {self.sensor_name}."
        )
    ...
    def export(self, msg_to_send: str) -> bool:
        ...
        try:
            slack_client.chat_postMessage(
                # Channel name is set in smartshield.yaml
                channel=self.slack_channel_name,
                # Sensor name is set in smartshield.yaml
