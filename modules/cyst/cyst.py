...
class Module(IModule):
    ...
    def init(self):
        ...
        self.cyst_UDS = "/run/smartshield.sock"
        self.conn_closed = False

    def initialize_unix_socket(self):
        """
        If the socket is there, smartshield will connect to itm if not, smartshield will create it
        """
        ...
        if not failure:
            self.print("smartshield is now listening. waiting for CYST to connect.")
        else:
    ...
    def shutdown_gracefully(self):
        ...
        # if smartshield is done, smartshield shouldn't expect more flows or send evidence
        # it should terminate
    ...
    def main(self):
        ...
        if flow := self.get_flow():
            # send the flow to inputprocess so smartshield can process it normally
            to_send = {
                "flow": flow,
