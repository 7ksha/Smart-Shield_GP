...
def get_ip_info_from_smartshield(ip_address: str, db) -> (float, float):
    """
    Get score and confidence on IP from smartshield.
    ...
    smartshield_score, smartshield_confidence = read_data_from_ip_info(ip_info)
    ...
    return (
        (None, None)
        if smartshield_score is None
        else (smartshield_score, smartshield_confidence)
    )
