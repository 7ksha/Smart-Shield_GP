...
class GoDirector:
    ...
    and from smartshield, and the response is sent.
    ...
    def process_go_data(self, report: dict) -> None:
        ...
        self.print(
            f"[The Network -> smartshield] Received msg {data} from peer {reporter}"
        )
        ...
        if message_type == "report":
            ...
            # TODO smartshield doesn't getthis kind of msgs at all. all reports are treated as one
    ...
    def process_message_request(
        ...
        Details are read from the request, and response is read from smartshield
         database.
        ...
        key = data["key"]
        self.print(
            f"[The Network -> smartshield] request about {key} from: {reporter}",
        )
    ...
    def respond_to_message_request(self, key, reporter):
        ...
        """
        score, confidence = get_ip_info_from_smartshield(key, self.db)
        if score is not None:
            send_evaluation_to_go(
                key, score, confidence, reporter, self.pygo_channel, self.db
            )
            self.print(
                f"[smartshield -> The Network] smartshield responded with info score={score} "
                f"confidence={confidence} about IP: {key} to {reporter}.",
                2,
                0,
            )
            # print(f"[smartshield -> The Network] smartshield responded with info score={score}
            # confidence={confidence} about IP: {key} to {reporter}.")
        else:
            self.print(
                f"[smartshield -> The Network] smartshield has no info about IP:"
    ...
    def process_message_report(
        ...
        if evaluation is not None:
            msg = (
                f"[The Network -> smartshield] Peer report about {key} "
    ...
    def process_evaluation_score_confidence(
        ...
        # with the width from smartshield.yaml and the starttime as the report time
        if key_type == "ip":
    ...
    def process_go_update(self, data: dict) -> None:
        ...
        try:
            ...
            msg = (
                f"[The Network -> smartshield] Peer update or new peer {peerid} "
