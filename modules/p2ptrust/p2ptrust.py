...
def validate_smartshield_data(message_data: str) -> (str, int):
    """
    ...
    :param message_data: data from smartshield request channel
...
class Trust(IModule):
    ...
    pigeon_binary = os.path.join(os.getcwd(), "p2p4smartshield/p2p4smartshield")
    ...
    def init(self, *args, **kwargs):
        ...
        # when restarting smartshield, it will look for the old keys in the new
        # output dir! so it wont find them and will
        # generate new keys, and therefore new peerid!
        # store the keys in smartshield main dir so they don't change every run
        ...
        # channel to send msgs to whenever smartshield needs
        ...
        # todo don't duplicate this dict, move it to smartshield_utils
        # all evidence smartshield detects has threat levels of strings
        ...
        # flag to ensure smartshield prints multiaddress only once
    ...
    def _configure(self):
        ...
        if self.start_pigeon:
            if not shutil.which(self.pigeon_binary):
                self.print(
                    f'P2p4smartshield binary not found in "{self.pigeon_binary}". '
    ...
    def new_evidence_callback(self, msg: Dict[str, str]):
        """
        Decides to share an evidence generated by smartshield to other peers or not
        ...
        # compare smartshield data with data in go
    ...
    def gopy_callback(self, msg: Dict):
        """
        this function is called whenever smartshield receives peers requests/updates
    ...
    #     try:
    #         data = msg["data"]
    #         self.print(f"IP info was updated in smartshield for ip: {data}")
    ...
    def set_evidence_malicious_ip(
        ...
        if "src" in ip_info.get("ip_state"):
            description = (
                ...
                f"to {saddr} Source: smartshield P2P network."
            )
        else:
            description = (
                f"Connection to blacklisted IP {attacker_ip} "
                f"from {saddr} Source: smartshield P2P network."
            )
    ...
    def handle_data_request(self, message_data: str) -> None:
        """
        Process the request from smartshield, ask the network and
        ...
        # nothing happens and smartshield should just check the database
        (
            cached_score,
            cached_confidence,
        ...
        self.print(f"[smartshield -> The Network] request about {ip_address}")
    ...
    def pre_main(self):
        ...
        # self.c4 = self.db.subscribe(self.smartshield_update_channel)
