...
from smartshield_files.core.helpers.whitelist.whitelist import Whitelist
...
class UpdateManager(IModule):
    ...
    def init(self):
        ...
        self.smartshield_logfile = self.db.get_stdfile("stdout")
        self.org_info_path = "smartshield_files/organizations_info/"
        self.path_to_mac_db = "databases/macaddress-db.json"
        # if any keyword of the following is present in a line
        # then this line should be ignored by smartshield
    ...
    def read_configuration(self):
        ...
        if not os.path.exists(self.path_to_remote_ti_files):
            ...
            # smartshield is started using sudo, it drops privs from modules that
    ...
    def get_feed_details(self, feeds_path):
        ...
        try:
            ...
            self.print(
                f"Error finding {feeds_path}. Feed won't be added to smartshield."
            )
            return {}
        ...
        for line in feeds.splitlines():
            ...
            if not utils.is_valid_threat_level(threat_level):
                ...
                self.print(
                    f"Invalid threat level found in smartshield.conf: {threat_level} "
    ...
    def log(self, text):
        """
        sends the text to output process to log it to smartshield.log without
         printing to the cli
        """
    ...
    def read_ports_info(self, ports_info_filepath) -> int:
        """
        Reads port info from smartshield_files/ports_info/ports_used_by_specific_orgs.csv
        and store it in the db
        """

        # there are ports that are by default considered unknown to smartshield,
        # but if it's known to be used by a specific organization, smartshield won't
        # consider it 'unknown'.
    ...
    def check_if_update_local_file(self, file_path: str) -> bool:
        ...
        hash changed
        for example: files in smartshield_files/ports_info
        """
    ...
    def should_update_online_whitelist(self) -> bool:
        ...
        Used for online whitelist specified in smartshield.conf
        """
    ...
    def parse_json_ti_feed(self, link_to_download, ti_file_path: str) -> bool:
        """
        smartshield has 2 json TI feeds that are parsed differently. hole.cert.pl
        and rstcloud
        """
    ...
    def is_valid_ti_file(self, ti_file_path: str) -> bool:
        ...
        try:
            ...
            # happens in integration tests, another instance of smartshield
            # deleted the file
            return False
    ...
    def normalize_line(self, ti_file_path: str, line: str) -> str:
        ...
        this fun extracts a format that smartshield can understand from this file
        """
    ...
    def extract_ip_info(
        ...
        try:
            ...
            # ones given in smartshield.conf
    ...
    def update_ports_info(self):
        for file in os.listdir("smartshield_files/ports_info"):
            file = os.path.join("smartshield_files/ports_info", file)
    ...
    def update_online_whitelist(self):
        """
        Updates online tranco whitelist defined in smartshield.yaml
    ...
    def should_update_mac_db(self) -> bool:
        """
        checks whether or not smartshield should download the mac db based on
    ...
    def delete_unused_cached_remote_feeds(self):
        """
        smartshield caches all the feeds it downloads. If the user deleted any of
    ...
    async def update(self) -> bool:
        ...
        we update different types of files remote TI files, remote JA3 feeds,
         RiskIQ domains and local smartshield files
        """
    ...
    async def update_ti_files(self):
        """
        Update TI files and store them in database before smartshield starts
        """
    ...
    def pre_main(self):
        ...
        try:
            # only one instance of smartshield should be able to update TI files at a time
            # so this function will only be allowed to run from 1 smartshield instance.
            with Lock(name="smartshield_macdb_and_whitelist_and_TI_files_update"):
            ...
            # another instance of smartshield is updating TI files, tranco
