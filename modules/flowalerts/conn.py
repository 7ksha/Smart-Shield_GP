...
from smartshield_files.common.abstracts.iflowalerts_analyzer import (
    IFlowalertsAnalyzer,
)
...
from smartshield_files.common.flow_classifier import FlowClassifier
...
class Conn(IFlowalertsAnalyzer):
    def init(self):
        ...
        # smartshield will alert data upload
    ...
    def port_belongs_to_an_org(self, daddr, portproto, profileid):
        ...
        4. match the IPs to orgs that smartshield has info about (apple, fb,
        google,etc.)
        """
        ...
        for org_name in organization_info["org_name"]:
            ...
            # if it's an org that smartshield has info about (apple, fb, google,etc.),
    ...
    def check_unknown_port(self, profileid, twid, flow):
        ...
        smartshield_files/ports_info/services.csv
        """
        if not flow.dport:
            return
    ...
    def check_device_changing_ips(self, twid, flow):
        ...
        this check is only done once for each private source ip smartshield sees
        """
    ...
    def is_interface_timeout_reached(self) -> bool:
        ...
        starting smartshield because the dns may have happened before starting smartshield
        """
        if not self.is_running_non_stop:
            # no timeout
            return True

        start_time = self.db.get_smartshield_start_time()
        now = datetime.now()
    ...
    async def check_connection_without_dns_resolution(
        ...
        # There is no DNS resolution, but it can be that smartshield is
        # still reading it from the files.
        # To give time to smartshield to read all the files and get all the flows
    ...
    def _is_ok_to_connect_to_ip_outside_localnet(self, flow) -> bool:
        ...
        for ip in (flow.saddr, flow.daddr):
            ip_obj = ipaddress.ip_address(ip)
            return (
                # because smartshield only knows about the ipv4 local networks
