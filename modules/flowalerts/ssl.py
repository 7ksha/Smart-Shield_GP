...
from smartshield_files.core.flows.suricata import SuricataTLS
from smartshield_files.core.flows.zeek import SSL
...
class SSL(IFlowalertsAnalyzer):
    ...
    def detect_incompatible_cn(self, twid, flow):
        ...
        if not org_found_in_cn:
            # the certificate doesn't claim to belong to any of smartshield known
            # orgs
            return False
    ...
    async def check_non_ssl_port_443_conns(
        ...
        # ts not reached
        # wait 5 mins real-time (to give smartshield time to
    ...
    async def wait_for_new_flows_or_timeout(self, timeout: float):
        ...
        # repeatedly check if smartshield is no longer receiving new flows
        async def will_smartshield_have_new_incoming_flows():
            """if smartshield will have no incoming flows, aka profiler stopped.
            this function will return False immediately"""
            while self.db.will_smartshield_have_new_incoming_flows():
                await asyncio.sleep(1)  # sleep to avoid busy looping
            return False

        try:
            # wait until either:
            # - will_smartshield_have_new_incoming_flows() returns False (no new flows)
            # - timeout is reached (5 minutes)
            await asyncio.wait_for(
                will_smartshield_have_new_incoming_flows(), timeout
            )
