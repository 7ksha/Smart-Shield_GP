...
from smartshield_files.core.flows.zeek import SMTP
...
class SetEvidenceHelper:
    ...
    def conn_without_dns(self, twid, flow) -> None:
        ...
        # is 0.1 in case of interface only, until smartshield learns all the DNS
        start_time: str = self.db.get_smartshield_start_time()
        now = datetime.now()
    ...
    def ssh_pw_guessing(self, flow, twid, uids: List[str]):
        confidence: float = 1.0
        description = (
            f"SSH password guessing to IP {flow.daddr}. Detected " f"by smartshield"
        )
        twid_number: int = int(twid.replace("timewindow", ""))
        evidence: Evidence = Evidence(
            evidence_type=EvidenceType.PASSWORD_GUESSING,
