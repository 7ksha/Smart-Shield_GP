...
from smartshield_files.common.parsers.config_parser import (
    ConfigParser,
)
...
from ..fidesModule.model.threat_intelligence import smartshieldThreatIntelligence
...
from ..fidesModule.persistence.threat_intelligence_db import (
    smartshieldThreatIntelligenceDatabase,
)
from ..fidesModule.persistence.trust_db import smartshieldTrustDatabase
from ..fidesModule.persistence.sqlite_db import SQLiteDB


class FidesModule(IModule):
    """
    This module ony runs when smartshield is running on an interface
    """
    ...
    def init(self):
        ...
        self.s2f = self.db.subscribe("smartshield2fides")
        self.ch_alert = self.db.subscribe("new_alert")
        self.f2s = self.db.subscribe("fides2smartshield")
        self.ch_ip = self.db.subscribe("new_ip")
        self.channels = {
            "network2fides": self.n2f,
            "fides2network": self.f2n,
            "smartshield2fides": self.s2f,
            "fides2smartshield": self.f2s,
    ...
    def __network_opinion_callback(self, ti: smartshieldThreatIntelligence):
        """This is executed every time when trust model was able to create an
        aggregated network opinion."""
        self.db.publish("fides2smartshield", json.dumps(ti.to_dict()))
