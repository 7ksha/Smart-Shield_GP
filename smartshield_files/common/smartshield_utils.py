...
class Utils(object):
    ...
    def __init__(self):
        ...
        # everyone
        self.smartshield_locks_dir = "/tmp/smartshield"
        self.time_formats = (
            "%Y-%m-%dT%H:%M:%S.%f%z",
            "%Y-%m-%d %H:%M:%S.%f",
            "%Y-%m-%d %H:%M:%S",
        ...
        # this format will be used across all modules and logfiles of smartshield
        # its timezone aware
    ...
    def get_interface_of_ip(self, ip, db, args) -> str:
        ...
        return s None if smartshield isnt running on an interface
        """
        if args.interface:
    ...
    def get_all_interfaces(self, args) -> List[str]:
        """
        returns a list of all interfaces smartshield is now monitoring
        :param args: smartshield args
        """
        if args.interface:
            return [args.interface]
        if args.access_point:
    ...
    def get_smartshield_version(self) -> str:
    ...
    def change_logfiles_ownership(self, file: str, UID, GID):
        """
        if smartshield is running in docker, the owner of the alerts log files
        is always root
        this function changes it to the user ID and GID in smartshield.yaml to be
         able to
