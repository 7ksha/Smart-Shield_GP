...
from smartshield_files.common.parsers.arg_parser import ArgumentParser
...
class ConfigParser(object):
    ...
    def read_config_file(self, configfile: str) -> dict:
        """
        reads smartshield configuration file, smartshield.conf/smartshield.yaml is the default file
        """
    ...
    def get_parser(self, help=False):
        return ArgumentParser(
            usage="./smartshield.py -c <configfile> [options]", add_help=help
        )

    def get_args(self):
        """
        Returns the args given to smartshield parsed by ArgumentParser
        """
        parser = self.get_parser()
    ...
    def read_configuration(self, section, name, default_value):
        """
        Read the configuration file for what smartshield.py needs.
    ...
    def get_entropy_threshold(self):
        """
        gets the shannon entropy used in detecting C&C over DNS TXT records from smartshield.conf/smartshield.yaml
        """
    ...
    def logsfile(self):
        return self.read_configuration("modes", "logsfile", "smartshield.log")

    def stdout(self):
        return self.read_configuration("modes", "stdout", "smartshield.log")
    ...
    def analysis_direction(self):
        """
        Controls which traffic flows are processed and analyzed by smartshield.

        Determines whether smartshield should focus on:
