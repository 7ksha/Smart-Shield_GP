...
from smartshield_files.core.database.redis_db.constants import (
    Constants,
    Channels,
)
from smartshield_files.core.database.redis_db.ioc_handler import IoCHandler
from smartshield_files.core.database.redis_db.alert_handler import AlertHandler
from smartshield_files.core.database.redis_db.profile_handler import ProfileHandler
from smartshield_files.core.database.redis_db.p2p_handler import P2PHandler
...
class RedisDB(IoCHandler, AlertHandler, ProfileHandler, P2PHandler):
    ...
    def __new__(
        ...
        if cls.redis_port not in cls._instances:
            ...
            # By default the smartshield internal time is
            # 0 until we receive something
            cls.set_smartshield_internal_time(0)
            if not cls.get_smartshield_start_time():
                cls._set_smartshield_start_time()

            # useful for debugging using 'CLIENT LIST' redis cmd
            cls.r.client_setname("smartshield-DB")
    ...
    def _set_redis_options(cls):
        """
        Updates the default smartshield options based on the -s param,
        ...
        # occurs without throwing errors in smartshield
        cls._options = {
            "daemonize": "yes",
        ...
        # save the db to <smartshield-dir>/dump.rdb
        cls._options.update(
            {
                "save": "30 500",
                "appendonly": "yes",
    ...
    @classmethod
    def set_smartshield_internal_time(cls, timestamp):
        """
        smartshield internal time is the timestamp of the last tw update done in
        smartshield
        it is updated each time smartshield detects a new modification in any
        timewindow
        metadata_manager.py checks for new tw modifications every 5s and
        updates this value accordingly
        """
        cls.r.set(cls.constants.smartshield_INTERNAL_TIME, timestamp)

    @classmethod
    def get_smartshield_start_time(cls) -> str:
        """get the time smartshield started in unix format"""
        return cls.r.get(cls.constants.smartshield_START_TIME)
    ...
    def init_redis_server(cls) -> Tuple[bool, str]:
        ...
        try:
            ...
            if cls.should_flush_db():
                ...
                # the PIDS to close smartshield files
                cls.r.flushdb()
    ...
    def connect_to_redis_server(cls) -> Tuple[bool, str]:
        ...
        """
        try:
            # db 0 changes everytime we run smartshield
    ...
    @classmethod
    def _set_smartshield_start_time(cls):
        """store the time smartshield started (datetime obj)"""
        now = time.time()
        cls.r.set(cls.constants.smartshield_START_TIME, now)
    ...
    def publish_stop(self):
        """
        Publish stop command to terminate smartshield
        to shutdown smartshield gracefully, this function should only be used by smartshield.py
        """
        self.print("Sending the stop signal to all listeners", 0, 3)
        self.r.publish("control_channel", "stop_smartshield")
    ...
    def get_smartshield_internal_time(self):
        return self.r.get(self.constants.smartshield_INTERNAL_TIME) or 0

    def set_ap_info(self, interfaces: Dict[str, str]):
        """the main smartshield instance call this func for the modules to be
        aware that smartshield is running as an access point"""
        return self.r.set(
            self.constants.IS_RUNNING_AS_AP, json.dumps(interfaces)
        )

    def get_ap_info(self) -> Dict[str, str] | None:
        """returns both AP interfaces or None if smartshield is not
        running in AP mode
        returns a dict with {"wifi_interface": <wifi>,
        "ethernet_interface": <eth0>}
        or None if smartshield is not running as an AP
        """
    ...
    def get_dns_resolution(self, ip: str):
        """
        IF this IP was resolved by smartshield
    ...
    def set_dns_resolution(
        ...
        for answer in answers:
            ...
            # we store ALL dns resolutions seen since starting smartshield
    ...
    def set_smartshield_mode(self, smartshield_mode):
        """
        function to store the current mode (daemonized/interactive)
        in the db
        """
        self.r.set(self.constants.MODE, smartshield_mode)

    def get_smartshield_mode(self):
        """
    ...
    def get_modified_ips_in_the_last_tw(self):
        """
        this number is updated in the db every 5s by smartshield.py
        used for printing running stats in smartshield.py or outputprocess
        """
    ...
    def mark_connection_error_as_logged(self):
        ...
        every module from logging it to smartshield.log and the console,
    ...
    def was_ip_seen_in_connlog_before(self, ip) -> bool:
        """
        returns true if this is not the first flow smartshield sees of the given ip
        """
    ...
    def is_growing_zeek_dir(self):
        """Did smartshield mark the given dir as growing?"""
    ...
    def _get_gw_info(self, interface: str) -> Dict[str, str] | None:
        """
        gets the gw of the given interface, when smartshield is runnuning on a
    ...
    def is_running_non_stop(self) -> bool:
        """
        smartshield runs non-stop in case of an interface or a growing zeek dir,
    ...
    def store_zeek_path(self, path):
        ...
        files smartshield is currently using"""
        self.r.set(self.constants.ZEEK_PATH, path)

    def get_zeek_path(self) -> str:
        """return the path of zeek log files smartshield is currently using"""
