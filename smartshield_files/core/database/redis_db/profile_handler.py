...
class ProfileHandler:
    ...
    def set_new_incoming_flows(self, will_smartshield_have_more_flows: bool):
        """A flag indicating if smartshield is still receiving new flows from
        input an profiler or not"""
        self.r.set(
            self.constants.WILL_smartshield_HAVE_MORE_FLOWS,
            "yes" if will_smartshield_have_more_flows else "no",
        )

    def will_smartshield_have_new_incoming_flows(self):
        """A flag indicating if smartshield is still receiving new flows from
        input an profiler or not"""
        return self.r.get(self.constants.WILL_smartshield_HAVE_MORE_FLOWS) == "yes"
    ...
    def _was_flow_flipped(self, flow) -> bool:
        ...
        here and it won't affect the rest of smartshield
        """
    ...
    def check_tw_to_close(self, close_all=False):
        ...
        were modified with the smartshield internal time
        :param close_all: close all tws no matter when they were last modified
        """

        sit = self.get_smartshield_internal_time()

        # sit is the ts of the last tw modification detected by smartshield
    ...
    def mark_profile_tw_as_modified(self, profileid, twid, timestamp):
        ...
        2- Add the timestamp received to the time_of_last_modification
           in the TW itself
        3- To update the internal time of smartshield
