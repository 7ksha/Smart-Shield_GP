...
from smartshield_files.core.database.redis_db.database import RedisDB
from smartshield_files.core.database.sqlite_db.database import SQLiteDB
from smartshield_files.common.parsers.config_parser import ConfigParser
from smartshield_files.core.structures.evidence import Evidence
from smartshield_files.core.structures.alerts import Alert
...
class DBManager:
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if self.conf.use_local_p2p():
            # import this on demand because smartshield light version doesn't
            # include the P2P dir
    ...
    def discard_obj(cls):
        ...
        that smartshield should connect to
        """
        cls._obj = None
    ...
    def get_smartshield_internal_time(self, *args, **kwargs):
        return self.rdb.get_smartshield_internal_time(*args, **kwargs)
    ...
    def set_smartshield_mode(self, *args, **kwargs):
        return self.rdb.set_smartshield_mode(*args, **kwargs)

    def get_smartshield_mode(self, *args, **kwargs):
        return self.rdb.get_smartshield_mode(*args, **kwargs)
    ...
    def will_smartshield_have_new_incoming_flows(self, *args, **kwargs):
        return self.rdb.will_smartshield_have_new_incoming_flows(*args, **kwargs)
    ...
    def _get_evidence_interface(self, evidence: Evidence) -> str:
        ...
        able to set an evidence without an interface. if smartshield is
        completely unable to return the used interface, it returns "default"
        """
        # when smartshield is only monitoring 1 interface, must be it
        ...
        if len(self.all_interfaces) == 1:
            ...
            # smartshield is running with -ap
            try:
    ...
    def update_threat_level(
        ...
        if self.trust_db:
            ...
            self.trust_db.insert_smartshield_score(
    ...
    def mark_as_ot_protected(self, *args, **kwargs):
        return self.rdb.mark_as_ot_protected(*args, **kwargs)

    def is_ot_protected(self, *args, **kwargs):
        return self.rdb.is_ot_protected(*args, **kwargs)

    def get_ot_protected_ips(self, *args, **kwargs):
        return self.rdb.get_ot_protected_ips(*args, **kwargs)
    ...
    def get_smartshield_start_time(self):
        return self.rdb.get_smartshield_start_time()

    def set_smartshield_internal_time(self, ts):
        return self.rdb.set_smartshield_internal_time(ts)
    ...
    def close_sqlite(self, *args, **kwargs):
        # when stopping the daemon using -S, smartshield doesn't start the sqlite db
        if self.sqlite:
