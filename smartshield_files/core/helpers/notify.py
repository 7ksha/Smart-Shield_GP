...
class Notify:
    ...
    def setup_notifications(self):
        ...
        Get the used display, the user using this display and the uid of this
         user in case of using smartshield as root on linux
        """
        ...
        # set to 0), if not we should know which screen is smartshield running on.
    ...
    def show_popup(self, alert_to_log: str):
        ...
        if platform.system() == "Linux":
            ...
            os.system(f'{self.notify_cmd} "smartshield" "{alert_to_log}"')
        elif platform.system() == "Darwin":
            os.system(
                f"osascript -e 'display notification"
                f' "{alert_to_log}" with title "smartshield"\' '
            )
