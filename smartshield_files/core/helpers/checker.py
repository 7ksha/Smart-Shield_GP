...
class Checker:
    ...
    def get_input_type(self) -> tuple:
        ...
        if self.main.args.input_module:
            ...
            # this is the default value of the type of flows smartshield reads from
            # a module
            line_type = "zeek"
            return input_type, input_information, line_type
    ...
    def _print_help_and_exit(self):
        """prints the help msg and shutd down smartshield"""
    ...
    def _check_if_growing_zeek_dir_is_used_correctly(self):
        ...
        if not self.main.args.interface:
            print(
                f"{usage}\n"
                "You need to define an interface with -i. Stopping smartshield"
            )
            self.main.terminate_smartshield()

        if self.main.args.filepath:
            print(f"{usage}\n" "-f shouldn't be used with -g. Stopping smartshield")
            self.main.terminate_smartshield()
    ...
    def _check_if_root_is_required(self):
        ...
        if (
            ...
            # iptables
            print("Run smartshield with sudo to use the blocking modules.")
            self.main.terminate_smartshield()
            return
        ...
        if self.main.args.clearblocking:
            if os.geteuid() != 0:
                print(
                    "smartshield needs to be run as root to clear the smartshieldBlocking "
                    "chain. Stopping."
                )
            else:
    ...
    def _is_smartshield_running_non_stop(self) -> bool:
        """determines if smartshield is monitoring real time traffic based oin
        the giving params"""
        return (
            self.main.args.interface
    ...
    def verify_given_flags(self):
        ...
        if (
            ...
            print("Redis database is not running. Stopping smartshield")
            self.main.terminate_smartshield()
            return

        if self.main.args.config and not os.path.exists(self.main.args.config):
            print(f"{self.main.args.config} doesn't exist. Stopping smartshield")
            self.main.terminate_smartshield()
        ...
        # Clear cache if the parameter was included
        if self.main.args.blocking and not self._is_smartshield_running_non_stop():
            print(
                "Blocking is only allowed when running smartshield on real time "
    ...
    def delete_blocking_chain(self):
        from modules.blocking.smartshield_chain_manager import (
            del_smartshield_blocking_chain,
        )
    ...
    def input_module_exists(self, module):
        """
        :param module: this is the one given to smartshield via --input-module
        check if the module was created in modules/ dir
        """
        available_modules = os.listdir("modules")

        if module not in available_modules:
            print(f"{module} module is not available. Stopping smartshield")
            return False
