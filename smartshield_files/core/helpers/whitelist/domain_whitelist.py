...
class DomainAnalyzer(IWhitelistAnalyzer):
    ...
    def is_whitelisted(
        ...
        # is the parent domain in any of smartshield whitelists?? like tranco or
        # whitelist.conf?
        # if so we need to get extra info about that domain based on the
        # whitelist.
        # e.g  by default smartshield whitelists all evidence and alerts from and to
        # tranco domains.
        ...
        if self.is_domain_in_tranco_list(parent_domain):
            ...
            # did the user say smartshield should ignore flows or alerts in the
            # config file?
            whitelist_should_ignore = domain_info["what_to_ignore"]
            # did the user say smartshield should ignore flows/alerts  TO or from
