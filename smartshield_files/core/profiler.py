...
from smartshield_files.common.abstracts.iobserver import IObservable
...
from smartshield_files.core.helpers.flow_handler import FlowHandler
from smartshield_files.core.helpers.symbols_handler import SymbolHandler
from smartshield_files.core.helpers.whitelist.whitelist import Whitelist
from smartshield_files.core.input_profilers.argus import Argus
from smartshield_files.core.input_profilers.nfdump import Nfdump
from smartshield_files.core.input_profilers.suricata import Suricata
from smartshield_files.core.input_profilers.zeek import ZeekJSON, ZeekTabs
...
class Profiler(ICore, IObservable):
    ...
    def add_flow_to_profile(self, flow):
        ...
        and does all the magic to convert it into a working data in smartshield.
        ...
        if self.db.is_cyst_enabled():
            ...
            # the user to know that smartshield is working
    ...
    def store_features_going_in(self, profileid: str, twid: str, flow):
        """
        If we have the all direction set , smartshield creates profiles
    ...
    def handle_in_flow(self, flow):
        ...
        """
        # they are not actual flows to add in smartshield,
    ...
    def get_input_type(self, line: dict, input_type: str):
        ...
        given to smartshield using -f and the name of the logfile this line was read from
        :the input_type: as determined by smartshield.py
        """
    ...
    def mark_process_as_done_processing(self):
        ...
        smartshield.py would know when to terminate
        """
    ...
    def get_localnet_of_given_interface(self) -> Dict[str, str]:
        """
        returns the local network of the given interface only if smartshield is
        running with -i
        """
        local_nets = {}
    ...
    def get_local_net_of_flow(self, flow) -> Dict[str, str]:
        ...
        """
        local_net = {}
        # Reaching this func means smartshield is running on a file. we either
