...
from smartshield_files.core.flows.argus import ArgusConn
...
class Argus(IInputType):
    ...
    def define_columns(self, new_line: dict) -> dict:
        ...
        line = new_line["data"]
        self.column_idx = {}
        # these are the fields as smartshield understands them
        # {original_argus_field_name: smartshield_field_name}
        supported_fields = {
            "time": "starttime",
            "endtime": "endtime",
            "appproto": "appproto",
        ...
        try:
            nline = line.strip().split(self.separator)
            # parse the given nline, and try to map the fields we find to the fields smartshield
            # undertsands from the dict above.
            for field in nline:
                for original_field, smartshield_field in supported_fields.items():
                    if original_field in field.lower():
                        # found 1 original field that smartshield supports. store its' smartshield
                        # equivalent name and index in the column_index
                        self.column_idx[smartshield_field] = nline.index(field)
                        break
            return self.column_idx
        except Exception:
