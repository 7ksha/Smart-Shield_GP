...
from smartshield_files.common.abstracts.iinput_type import IInputType
from smartshield_files.common.smartshield_utils import utils
from smartshield_files.core.flows.zeek import (
    Conn,
    DNS,
    HTTP,
    SSL,
...
from smartshield_files.core.input_profilers.zeek_to_smartshield_maps import (
    conn_fields_to_smartshield_fields_map,
    dns_fields_to_smartshield_fields_map,
    http_fields_to_smartshield_fields_map,
    ssl_fields_to_smartshield_fields_map,
    ssh_fields_to_smartshield_fields_map,
    dhcp_fields_to_smartshield_fields_map,
    ftp_fields_to_smartshield_fields_map,
    smtp_fields_to_smartshield_fields_map,
    tunnel_fields_to_smartshield_fields_map,
    notice_fields_to_smartshield_fields_map,
    files_fields_to_smartshield_fields_map,
    arp_fields_to_smartshield_fields_map,
    software_fields_to_smartshield_fields_map,
    weird_fields_to_smartshield_fields_map,
)

LOG_MAP = {
    "conn.log": conn_fields_to_smartshield_fields_map,
    "dns.log": dns_fields_to_smartshield_fields_map,
    "http.log": http_fields_to_smartshield_fields_map,
    "ssl.log": ssl_fields_to_smartshield_fields_map,
    "ssh.log": ssh_fields_to_smartshield_fields_map,
    "dhcp.log": dhcp_fields_to_smartshield_fields_map,
    "ftp.log": ftp_fields_to_smartshield_fields_map,
    "smtp.log": smtp_fields_to_smartshield_fields_map,
    "tunnel.log": tunnel_fields_to_smartshield_fields_map,
    "notice.log": notice_fields_to_smartshield_fields_map,
    "files.log": files_fields_to_smartshield_fields_map,
    "arp.log": arp_fields_to_smartshield_fields_map,
    "software.log": software_fields_to_smartshield_fields_map,
    "weird.log": weird_fields_to_smartshield_fields_map,
}

# define a mapping of log types to their corresponding classes
LINE_TYPE_TO_smartshield_CLASS = {
    "conn.log": Conn,
    "dns.log": DNS,
    "http.log": HTTP,
    "ssl.log": SSL,
...
class Zeek:
    ...
    def get_file_type(self, new_line: dict) -> str:
        """
        returnx x.log. always. no atter whats the name given to smartshield
        """
        ...
        # if the zeek dir given to smartshield has 'conn' in it's name,
        # smartshield thinks it's reading a conn file
    ...
    def fill_empty_class_fields(self, flow_values: dict, smartshield_class):
        """
        The given smartshield class is Conn, SSH, Weird etc.
        ...
        the given smartshield_class

        :param flow_values: a dict with the values of the given zeek log line
        :param smartshield_class: the class that corresponds to the given zeek log
        line
        """
        # get all fields of the smartshield_class
        smartshield_class_fields = set(smartshield_class.__init__.__code__.co_varnames)
        # remove 'self' from the fields
        smartshield_class_fields.discard("self")

        # identify fields in smartshield_class that are not in flow_values
        missing_fields = smartshield_class_fields - set(flow_values.keys())

        # set the missing fields in flow_values to ""
        for field in missing_fields:
            flow_values[field] = ""

        # always use the type_ field of the smartshield class, this is not gonna
        # be given to smartshield by zeek:D
        flow_values["type_"] = getattr(smartshield_class, "type_")
...
class ZeekJSON(IInputType, Zeek):
    ...
    def process_line(self, new_line: dict):
        ...
        for zeek_field, smartshield_field in line_map.items():
            if not smartshield_field:
                continue
            val = line.get(zeek_field, "")
            if val == "-":
                val = ""
            flow_values[smartshield_field] = val

        if file_type in LINE_TYPE_TO_smartshield_CLASS:
            smartshield_class = LINE_TYPE_TO_smartshield_CLASS[file_type]
            ...
            flow_values = self.fill_empty_class_fields(
                flow_values, smartshield_class
            )
            self.flow = smartshield_class(**flow_values)
            return self.flow
...
class ZeekTabs(IInputType, Zeek):
    ...
    def get_smartshield_fields_idx_map(
        self, fields_line: dict, zeek_to_smartshield_field_map: dict
    ) -> Dict[int, str]:
        """
        returns a dict that has smartshield fields as values and the index of
        each field as keys
        e.g {0: 'starttime', 1: 'uid', 2: 'saddr' etc...
        :param fields_line: a line that starts with #fields as read from
        the given zeek.log file
        :param zeek_to_smartshield_field_map: a dict that maps zeek fields to smartshield
        the returned dict may contain None if a zeek field in the given
        line doesnt have a smartshield equivalent (not used in smartshield)
        """
        ...
        smartshield_fields_idx_map = {}
        for idx, zeek_field in enumerate(zeek_fields_list):
            # map field to smartshield name if possible
            smartshield_field = zeek_to_smartshield_field_map.get(zeek_field)
            smartshield_fields_idx_map[idx] = smartshield_field
        return smartshield_fields_idx_map
    ...
    def update_line_processor_cache(self, fields_line: dict) -> None:
        ...
        """
        zeek_to_smartshield_field_map: Dict[str, str] = self.get_line_processor(
            fields_line
        )

        indices_of_each_smartshield_field: Dict[int, str]
        indices_of_each_smartshield_field = self.get_smartshield_fields_idx_map(
            fields_line, zeek_to_smartshield_field_map
        )

        file_type: str = self.get_file_type(fields_line)
        self.line_processor_cache.update(
            {file_type: indices_of_each_smartshield_field}
        )
    ...
    def process_line(self, new_line: dict):
        ...
        if line.startswith("#fields"):
            ...
            # to some smartshield obj (SSL, Con, SSH, etc...)
            # cache the mapping for later use
            self.update_line_processor_cache(new_line)
            return

        file_type: str = self.get_file_type(new_line)
        # this dict is the name of each smartshield field and the index of it
        ...
        if not line_processor:
            print(
                f"smartshield is unable to handle the given zeek log line! "
                f"{new_line}",
                0,
                1,
            )
        ...
        if log_type in LINE_TYPE_TO_smartshield_CLASS:
            ...
            # Conn, SSH, Notice, etc.
            smartshield_class = LINE_TYPE_TO_smartshield_CLASS[log_type]
