...
from smartshield_files.common.abstracts.icore import ICore
...
from smartshield_files.common.style import yellow
from smartshield_files.core.helpers.filemonitor import FileEventHandler
from smartshield_files.core.supported_logfiles import SUPPORTED_LOGFILES
from smartshield_files.core.zeek_cmd_builder import ZeekCommandBuilder
...
class Input(ICore):
    ...
    def init(
        ...
        # in case of reading from stdin, the user must tell smartshield what
    ...
    def mark_self_as_done_processing(self):
        ...
        stop so smartshield.py would know when to terminate
        """
        # signal smartshield.py that this process is done
    ...
    def check_if_time_to_del_rotated_files(self):
        """
        After a specific period (keep_rotated_files_for), smartshield deletes all rotated files
    ...
    def get_file_handle(self, filename):
        ...
        try:
            ...
            try:
                ...
                # new dns request
                # occurs
                # if smartshield tries to read from the old dns.log now it won't
    ...
    def read_zeek_folder(self):
        ...
        - a finite zeek dir is given to smartshield with -f
        - a growing zeek dir is given to smartshield with -g
        This func does not run when smartshield is running on an interface with
        -i or -ap
        """
        # wait max 10 seconds before stopping smartshield if no new flows are read
        self.bro_timeout = 10
        growing_zeek_dir: bool = self.db.is_growing_zeek_dir()
        if growing_zeek_dir:
            # smartshield is given a dir that is growing i.e zeek dir running on an
            # interface
            # don't stop zeek or smartshield
            self.bro_timeout = float("inf")

        self.zeek_dir = self.given_path
        # if smartshield is just reading a finite zeek dir, there's no way to
        # know the interface
        interface = "default"
        ...
        for file in os.listdir(self.given_path):
            ...
            if not growing_zeek_dir:
                # get the total number of flows smartshield is going to read
    ...
    def read_from_stdin(self) -> bool:
        ...
        for line in self.stdin():
            ...
            if line == "done":
                break
            # smartshield supports reading zeek json conn.log only using stdin,
    ...
    def handle_zeek_log_file(self):
        """
        Handles conn.log files given to smartshield directly,
         and conn.log flows given to smartshield through CYST unix socket.
        """
        if (
    ...
    def handle_pcap_and_interface(self) -> bool:
        """
        runs when smartshield is given a pcap with -f, an interface with -i,
        ...
        if self.input_type == "interface":
            # smartshield is running with -i or -ap
            ...
            if self.args.interface:
                ...
                # smartshield is running in AP mode, we need to monitor the 2
    ...
    def init_zeek(
        ...
        keeps running until smartshield stops
        """
    ...
    def remove_old_zeek_files(self):
        ...
        it deletes old zeek-date.log files and clears smartshield' open handles and sleeps again
        """
        while not self.should_stop():
            # keep the rotated files for the period specified in smartshield.yaml
            if msg := self.get_msg("remove_old_files"):
                ...
                lock.acquire()
                try:
                    # close smartshield' open handles
                    ...
                    # we probably don't need it in smartshield
    ...
    def handle_cyst(self):
        ...
        """
        # smartshield supports reading zeek json conn.log only using CYST
        # this type is passed here by smartshield.py, so in the future
        # to support more types, modify smartshield.py
        if self.line_type != "zeek":
            return
    ...
    def main(self):
        if self.is_running_non_stop:
            ...
            # if it started from __init__() it will have the PID of smartshield.py therefore,
