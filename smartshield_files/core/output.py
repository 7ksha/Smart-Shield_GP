...
from smartshield_files.common.abstracts.iobserver import IObserver
...
from smartshield_files.common.style import red, yellow


class Output(IObserver):
    """
    A class to process and output all text to cli and to smartshield log files.
    If any smartshield module or process needs to print or log anything to screen,
     or logs, it should use The printer that uses this process.
    """

    name = "Output"
    smartshield_logfile_lock = Lock()
    ...
    def __init__(
        ...
        smartshield_logfile="output/smartshield.log",
        input_type=False,
        create_logfiles: bool = True,
        stdout="",
    ):
        super().__init__()
        # when running smartshield using -e , this var is set and we only
        ...
        self.smartshield_logfile = smartshield_logfile
        ...
        if create_logfiles:
            ...
            # root (if smartshield was started by root)
            os.umask(0)
            self._read_configuration()
            self.create_logfile(self.errors_logfile)
            self.log_branch_info(self.errors_logfile)
            self.create_logfile(self.smartshield_logfile)
            self.log_branch_info(self.smartshield_logfile)

            utils.change_logfiles_ownership(
                self.errors_logfile, self.UID, self.GID
            )
            utils.change_logfiles_ownership(
                self.smartshield_logfile, self.UID, self.GID
            )
    ...
    def create_logfile(self, path):
        """
        creates smartshield.log and errors.log if they don't exist
        """
        try:
            open(path, "a").close()
    ...
    def log_line(self, msg: dict):
        """
        Logs line to smartshield.log
        """

        # don't log in daemon mode, all printed
        # lines are redirected to smartshield.log by default
        if "-D" in sys.argv:
            return
        ...
        self.smartshield_logfile_lock.acquire()
        with open(self.smartshield_logfile, "a") as smartshield_logfile:
            smartshield_logfile.write(f"{date_time} [{sender}] {msg}\n")
        self.smartshield_logfile_lock.release()
    ...
    def output_line_to_cli_and_logfiles(self, msg: dict):
        ...
        # if the line is an error and we're running smartshield without -e 1 ,
