...
from smartshield_files.common.idmefv2 import IDMEFv2
from smartshield_files.common.style import (
    green,
)
...
from smartshield_files.core.evidence_logger import EvidenceLogger
from smartshield_files.core.helpers.whitelist.whitelist import Whitelist
from smartshield_files.core.helpers.notify import Notify
...
from smartshield_files.core.structures.alerts import (
    Alert,
)
from smartshield_files.core.text_formatters.evidence_formatter import (
    EvidenceFormatter,
)
...
class EvidenceHandler(ICore):
    ...
    def read_configuration(self):
        ...
        # In docker, disable alerts no matter what smartshield.yaml says
    ...
    def is_blocking_modules_supported(self) -> bool:
        """
        returns true if smartshield is running in an interface or growing
         zeek dir with -p
        or if smartshield is using custom flows (meaning smartshield is reading the
    ...
    def decide_blocking(
        ...
        Decide whether to block or not and send to the blocking module
         returns True if the given IP was blocked by smartshield blocking module
        """
    ...
    def should_stop(self) -> bool:
        ...
        # without this, smartshield has problems processing the last evidence
    ...
    def main(self):
        while not self.should_stop():
            ...
            if msg := self.get_msg("new_blame"):
                ...
                ip_info = {"p2p4smartshield": evaluation}
                ip_info["p2p4smartshield"].update({"ts": time.time()})
