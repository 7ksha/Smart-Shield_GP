...
class Daemon:
    description = "This module runs when smartshield is in daemonized mode"
    name = "Daemon"

    def __init__(self, smartshield):
        ...
        self.smartshield.set_mode("daemonized", daemon=self)
        # this is a conf file used to store the pid of the daemon and
        # is deleted when the daemon stops
        self.pidfile_dir = "/var/lock"
        self.daemon_start_lock = "smartshield_daemon_start"
        self.daemon_stop_lock = "smartshield_daemon_stop"
        self.pidfile = os.path.join(self.pidfile_dir, "smartshield_daemon.lock")
        self.read_configuration()
        if not self.smartshield.args.stopdaemon:
    ...
    def print(self, text, **kwargs):
        """Prints output to logsfile specified in smartshield.yaml"""
    ...
    def prepare_output_dir(self):
        if "-o" in sys.argv:
            self.prepare_std_streams(self.smartshield.args.output)
        else:
            # if we have acess to '/var/log/smartshield/' store the logfiles there,
            # if not , store it in the output/ dir
            try:
                output_dir = "/var/log/smartshield/"
                try:
                    os.mkdir(output_dir)
                ...
                self.smartshield.args.output = output_dir
    ...
    def daemonize(self):
        ...
        # because the parent and the first fork, already exited, so this pid is the daemon pid which is smartshield.py pid
    ...
    def start(self):
        """
        Main function
        Starts the daemon and starts smartshield normally.
        """
        try:
            with Lock(name=self.daemon_start_lock):
                ...
                # the daemon and have the same PID as smartshield.py
                self.print(f"smartshield Daemon is running. [PID {self.pid}]\n")
                self.smartshield.start()
        except CannotAcquireLock:
            # another instance of smartshield daemon is running
            return

    def get_last_opened_daemon_info(self) -> Optional[Tuple[str]]:
        """
        get information about the last opened smartshield daemon from
         running_smartshield_info.txt
        """
        try:
            with open(self.smartshield.redis_man.running_logfile, "r") as f:
                ...
                for line in f.read().splitlines()[::-1]:
                    ...
                    if not is_daemon:
                        continue
                    port, output_dir, smartshield_pid = line[2], line[5], line[6]
                    return port, output_dir, smartshield_pid
        except FileNotFoundError:
            # file removed after daemon started
            self.print(
                f"Warning: {self.smartshield.redis_man.running_logfile} is not found. Can't get daemon info."
                f" smartshield won't be completely killed."
            )

    def _kill_self(self):
        """Kill the daemon process only (aka smartshield.py)"""

        # sending SIGINT to self.pid will only kill smartshield.py
        # and the rest of its children will be zombies

        # sending SIGKILL to self.pid will only kill smartshield.py and the rest of
    ...
    def stop(self):
        ...
        try:
            with Lock(name=self.daemon_stop_lock):
                ...
                self.stdout = "smartshield.log"
                self.logsfile = "smartshield.log"
                self.prepare_std_streams(output_dir)

                self.logger = self.smartshield.proc_man.start_output_process(
                    self.stderr, self.logsfile, stdout=self.stdout
                )
                self.smartshield.printer = Printer(self.logger, self.name)

                self.db = DBManager(
                    self.logger,
                    output_dir,
                    int(port),
                    self.smartshield.conf,
                    self.smartshield.pid,
                    start_sqlite=False,
                    flush_db=False,
                )
                self.db.set_smartshield_mode("daemonized")
                self.smartshield.set_mode("daemonized", daemon=self)

                self.smartshield.input_information = self.db.get_input_file()
                self.smartshield.args = self.smartshield.conf.get_args()
                self.smartshield.db = self.db
                self.smartshield.proc_man.smartshield_logfile = self.logsfile
