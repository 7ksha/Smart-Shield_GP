...
from smartshield_files.common.style import green, yellow
...
from smartshield_files.core.helpers.checker import Checker
...
class Main:
    def __init__(self, testing=False):
        ...
        self.version = utils.get_smartshield_version()
        ...
        if not testing:
            ...
            if not self.args.stopdaemon:
                ...
                # this is the zeek dir smartshield will be using
    ...
    def terminate_smartshield(self):
        """
        Shutdown smartshield, is called when stopping smartshield before
        starting all modules. for example using -cb or -k
        """
        if self.mode == DAEMONIZED_MODE:
            self.daemon.stop()
        if not self.conf.get_cpu_profiler_enable():
            sys.exit(0)  # leaves any children started by smartshield as orphans
    ...
    def prepare_output_dir(self):
        ...
        if "-o" in sys.argv:
            ...
            if os.path.exists(self.args.output):
                for file in os.listdir(self.args.output):
                    # in integration tests, smartshield redirects its
                    # output to smartshield_output.txt,
                    # don't delete that file
                    if self.args.testing and "smartshield_output.txt" in file:
                        continue
        ...
        # now that smartshield can run several instances,
    ...
    def set_mode(self, mode, daemon=""):
        """
        smartshield has 2 modes, daemonized and interactive, this function
        sets up the mode so that smartshield knows in which mode it's operating
    ...
    def get_input_file_type(self, given_path):
        ...
        if (
            ...
            if shutil.which("nfdump") is None:
                # If we do not have nfdump, terminate smartshield.
                print("nfdump is not installed. terminating smartshield.")
                self.terminate_smartshield()
            ...
            for log_file in os.listdir(given_path):
                ...
                # given directory, start smartshield normally
                # otherwise, stop smartshield
    ...
    def print_version(self):
        smartshield_version = f"smartshield Version: {green(self.version)}"
        branch_info = utils.get_branch_info()
        if branch_info is not False:
            # it's false when we're in docker because there's no .git/ there
            self.commit, self.branch = branch_info
            smartshield_version += f" ({self.commit[:8]})"
        smartshield_version.replace("\n", "")
        print(smartshield_version)
    ...
    def is_total_flows_unknown(self) -> bool:
        """
        Determines if smartshield knows the total flows it's gonna be
    ...
    def get_smartshield_logfile(self) -> str:
        if self.mode == "daemonized":
            return self.daemon.stdout
        elif self.mode == "interactive":
            return os.path.join(self.args.output, "smartshield.log")

    def get_smartshield_error_file(self) -> str:
        if self.mode == "daemonized":
    ...
    def prepare_locks_dir(self):
        """
        sets the correct permissions for the /tmp/smartshield directory to be
        used by root and non-root users
        """
        locks_dir = utils.smartshield_locks_dir
    ...
    def start(self):
        """Main smartshield Function"""
        try:
            self.print_version()
            print("https://stratosphereips.org")
            print("-" * 27)
            self.setup_print_levels()
            stderr: str = self.get_smartshield_error_file()
            smartshield_logfile: str = self.get_smartshield_logfile()
            ...
            self.logger = self.proc_man.start_output_process(
                stderr, smartshield_logfile
            )
            self.printer = Printer(self.logger, self.name)
            self.print(f"Storing smartshield logs in {self.args.output}")
            ...
            if self.args.access_point:
                ...
                if not self.ap_manager.is_ap_running():
                    self.print(
                        "smartshield was started with -ap but can't detect a "
                        "running access point. Please start an access point "
                        "and restart smartshield. Stopping."
                    )
                    self.terminate_smartshield()
                else:
                    self.print(yellow("smartshield is running in AP mode."))
            ...
            self.db.set_smartshield_mode(self.mode)
            ...
            if self.mode == DAEMONIZED_MODE:
                ...
                std_files = {
                    "stderr": stderr,
                    "stdout": smartshield_logfile,
                }

            self.db.store_std_file(**std_files)

            # if smartshield is given a .rdb file, don't load the
            ...
            if not self.args.db:
                ...
                # smartshield will wait untill all TI files are updated before
            ...
            self.proc_man.set_smartshield_processes(children)
            ...
            if open_servers > 1:
                self.print(
                    ...
                    f"Run smartshield with --killall to stop them."
                )

            self.print(
                "Warning: smartshield may generate a large amount "
                "of traffic by querying TI sites."
            )

            # Don't try to stop smartshield if it's capturing from
            # an interface or a growing zeek dir
            while not self.proc_man.stop_smartshield():
                ...
                modified_profiles: Set[str] = (
                    self.metadata_man.update_smartshield_stats_in_the_db()[1]
                )
